...
export const RESTAURANT_FOODS: Food[] = [
  ...
  },
  {
    ...
    variant: "breast, toppings only — the rice base is its own entry",
    brand: "BIGBELLY Greek Street Food",
    source: "claude",
    per: "1 bowl's toppings (~420 g), no rice",
    gramsPerServing: 420,
    macros: {
      calories: 575,
      protein: 64.5,
    ...
    note: "BIGBELLY publishes no nutrition, and bigbellygreek.com was unreachable from this machine on August 20, 2026 — blocked at the network egress proxy — so this is reconstructed from the menu as described in listings and reviews rather than read off a panel. THIS ROW IS THE BOWL WITHOUT ITS RICE, deliberately: the rice is a separate entry so a partly-eaten base logs as a fraction of it instead of forcing a re-estimate of the whole dish. Built as roughly 190 g of grilled chicken breast souvlaki plus ~6 g of retained marinade oil, ~120 g of the cucumber, tomato, onion and olive salad that is mixed in, and the tzatziki and hummus served on the side at ~55 g each. THE CHICKEN IS THE WHOLE ROW: at ±30% on the meat (135–250 g cooked) the protein runs 42–73 g and the calories 450–700, and nothing else in the bowl moves the total nearly as much. 190 g sits at the generous end because huge portions in the large takeout container is the single most repeated observation about this place across Yelp and blogTO — but that is an inference from other people's plates, not a weighing, and if a normal-looking scoop of meat turns up next time this should come down. Sodium is the widest figure at 900–1,900 mg and comes from four places that cannot be separated by looking: the marinade ~500 mg, the kalamata olives and vinaigrette ~400, tzatziki ~180, hummus ~190. TWO THINGS ARE DELIBERATELY NOT IN HERE. The fried pita some bowls arrive with, which reviews mention and which would add roughly 120–150 kcal — log it separately if it was there. And the chosen sauce: BIGBELLY offers a list (feta jalapeno, creamy garlic, greek pesto, greek lemon and others) and a heavy pour of a creamy one is another 100–150 kcal and 200–300 mg of sodium, so a bowl ordered with one is a different row from this.",
